...
# 文件夹路径
folder_path = os.path.join(DATA_ROOT,"motion/human/SG")
...
for root, dirs, files in os.walk(folder_path):
    ...
    for idx, filename in enumerate(files):
        ...
        # 示例：使用wc命令统计文件行数
        if "bvh" in file_path:
            # result = subprocess.run(["python", os.path.join(SRC_ROOT,"retarget/sg_g1_15.py"), file_path], capture_output=True, text=True)
            print(file_path)
            # # 打印命令输出
