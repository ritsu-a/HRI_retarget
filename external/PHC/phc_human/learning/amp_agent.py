from phc_human.utils.running_mean_std import RunningMeanStd
...
from phc_human.env.tasks.humanoid_amp_task import HumanoidAMPTask
...
import copy
from phc_human.utils.torch_utils import project_to_norm
import learning.amp_datasets as amp_datasets
from phc_human.learning.loss_functions import kl_multi
...
class AMPAgent(common_agent.CommonAgent):
    ...
    def play_steps(self):
        ...
        mb_rewards = self.experience_buffer.tensor_dict['rewards']
        mb_amp_obs = self.experience_buffer.tensor_dict['amp_obs']
        amp_rewards = self._calc_amp_rewards(mb_amp_obs) # disc
        mb_rewards = self._combine_rewards(mb_rewards, amp_rewards)
        mb_advs = self.discount_values(mb_fdones, mb_values, mb_rewards, mb_next_values)
        mb_returns = mb_advs + mb_values
    ...
    def _assemble_train_info(self, train_info, frame):
        ...
        if 'terminated_flags' in train_info:
            train_info_dict["success_rate_training_epoch"] =  1 - torch.mean((train_info['terminated_flags'] > 0).float()).item()
