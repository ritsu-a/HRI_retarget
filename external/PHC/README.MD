...
```
python phc/run_hydra.py learning=im_mcp_big  exp_name=phc_comp_3 env=env_im_getup_mcp robot=smpl_humanoid env.zero_out_far=False robot.real_weight_porpotion_boxes=False env.num_prim=3 env.motion_file=data/amass/pkls/amass_isaac_test_0.pkl env.models=['output/HumanoidIm/phc_3/Humanoid.pth'] env.num_envs=1  headless=False epoch=-1 test=True
```
...
## keypoint model
python phc/run_hydra.py learning=im_mcp exp_name=phc_kp_mcp_iccv  test=True env=env_im_getup_mcp robot=smpl_humanoid robot.freeze_hand=True robot.box_body=False env.z_activation=relu env.motion_file=data/amass/pkls/amass_isaac_test_0.pkl env.models=['output/HumanoidIm/phc_kp_pnn_iccv/Humanoid.pth'] env.num_envs=1 env.obs_v=7 headless=False epoch=-1
...
```
python scripts/data_process/process_amass_raw.py --dir /cephfs_yili/backup/xuehan/dataset/amass --out_dir /cephfs_yili/shared/xuehan/PHC
```

which dumps the data into the `amass_db_smplh.pt` file. Then, run 

```
python scripts/data_process/process_amass_db.py --path /cephfs_yili/shared/xuehan/PHC/amass_db_smplh.pt
```

We further process these data into Motionlib format by running the following script:

```
python scripts/data_process/convert_amass_isaac.py --in_file /cephfs_yili/shared/xuehan/PHC/sample_data/amass_train_13912.pkl --out_file /cephfs_yili/shared/xuehan/H1_RL/amass_train_13912.pkl
```
...
```
CUDA_VISIBLE_DEVICES=1  python phc/run_hydra.py learning=im_pnn_big env=env_im_pnn robot=smpl_humanoid env.motion_file=data/amass/pkls/amass_isaac_im_train_take6_upright_slim.pkl exp_name=phc_test
```
...
```
python scripts/pmcp/forward_pmcp.py --exp phc_1 --epoch 24000 --idx 1
```
...
To train the next primitive, run the following script:
```
export exp_name=phc_1
export epoch=3000
export idx=1
python phc/run_hydra.py learning=im_pnn_big env=env_im_pnn robot=smpl_humanoid env.motion_file=data/amass/pkls/auto_pmcp/phc_1_3000.pkl epoch=3001 env.fitting=True env.training_prim=1 exp_name=phc_1
```
...
```
python phc/run_hydra.py learning=im_pnn_big env=env_im_pnn robot=smpl_humanoid env.motion_file=[motion_file] epoch=[include current epoch + 1 from the forward_pmcp step] env.fitting=True env.training_prim=[+1] env.zero_out_far=True env.zero_out_far_train=True env.getup_udpate_epoch={epoch} exp_name=phc_1
```

After all primitives are trained, train the composer: 

```
python phc/run_hydra.py learning=im_mcp_big exp_name=phc_0_cp env=env_im_getup_mcp robot=smpl_humanoid env.motion_file=data/amass/pkls/amass_isaac_im_train_take6_upright_slim.pkl env.models=['output/HumanoidIm/phc_0/Humanoid.pth']
```
