...
asset_root = "/home/zhikai/humanplus/HST/isaacgym/assets"
...
# load h1 motion data
motion_name = "ACCAD_walk_10fps.npy"
motion_data_path = "/home/zhikai/humanplus/HST/isaacgym/h1_motion_data/"
motion_data = np.load(motion_data_path + motion_name)
motion_data += np.array([ 0.0000,  0.0000, -0.3490,  0.6980, -0.3490,  0.0000,  0.0000, -0.3490,
          0.6980, -0.3490,  0.0000,  0.0000,  0.0000,  0.0000,  0.0000,  0.0000,
          0.0000,  0.0000,  0.0000])
motion_pos_data = np.expand_dims(motion_data, axis=-1)
motion_vel_data= np.zeros_like(motion_pos_data)
motion_vel_data[1: , :] = (motion_pos_data[1: , :] - motion_pos_data[:-1, :]) * save_fps
motion_vel_data[0, :]  = motion_vel_data[1, :]
# ipdb.set_trace()
...
gym.prepare_sim(sim)   
dof_cart_pos_list = []  
h1_joint_names = gym.get_actor_joint_names(envs[0], actor_handles[0])
for i in tqdm(range(motion_data.shape[0])):
    ...
    cur_joint_cart_pos = gym.get_actor_joint_transforms(envs[0], actor_handles[0])
    cur_dof_cart_pos_list = []
    for j in range(len(h1_dof_names)):
        cur_dof_cart_pos_list.append(np.array([cur_joint_cart_pos[h1_joint_names.index(h1_dof_names[j])][0][0], cur_joint_cart_pos[h1_joint_names.index(h1_dof_names[j])][0][1], cur_joint_cart_pos[h1_joint_names.index(h1_dof_names[j])][0][2]]))
    dof_cart_pos_list.append(np.stack(cur_dof_cart_pos_list))     
    if show:
    ...
    if show and gym.query_viewer_has_closed(viewer):
        break
dof_cart_pos = np.stack(dof_cart_pos_list)
dof_cart_vel= np.zeros_like(dof_cart_pos)
dof_cart_vel[1: , :] = (dof_cart_pos[1: , :] - dof_cart_pos[:-1, :]) * save_fps
dof_cart_vel[0, :]  = dof_cart_vel[1, :]
extended_h1_motion = np.concatenate([motion_pos_data, motion_vel_data, dof_cart_pos, dof_cart_vel], axis = -1)
pre_name = "h1_"
if save:
    np.save("../../h1_motion_data/" + "extended_" + motion_name, extended_h1_motion)
