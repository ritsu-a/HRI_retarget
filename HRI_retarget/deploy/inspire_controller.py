...
class Inspire_Controller:
    ...
    def set_qpos(self, hand_qpos):
        for i in range(Inspire_Num_Motors * 2):
            self.hand_msg.cmds[i].q = hand_qpos[i]
        self.HandCmb_publisher.Write(self.hand_msg)
    
    def get_qpos(self):
        hand_qpos = np.zeros(Inspire_Num_Motors * 2)
        for idx, id in enumerate(Inspire_Left_Hand_JointIndex):    
            hand_qpos[id.value] = self.left_hand_state_array[idx]
        for idx, id in enumerate(Inspire_Right_Hand_JointIndex):   
            hand_qpos[id.value] = self.right_hand_state_array[idx]
        return hand_qpos
